...
# This is much the same script as 01-build-data.R, but with different parameters: in the previous script, 
#   we pulled the training data from the snapshot at the beginning of the previous fiscal year, and the outcomes
#   from the end of that year, ie. did someone give or not.
#   In this script, we pull the current data, hoping to predict whether or not someone will give this fiscal year.
#
# Eventually, I need to roll these functions into mumodels. That's a project I'll probably tackled next time I rebuild the model.
#   For now, I re-iterate the functions, because often I'll save all my model data to a file, then clear out my environment to keep
#   my computer from crawling to a halt. 
...
showlevels  <- function(x) {
  
  y  <- ifelse(!is.factor(x), factor(x), x) 
...
# gotta call out muadc::currentFY in case you've got currentFY set to something else from building the training data!
buildvars  <- make_build_vars(type = "predictors", thisfy = muadc::currentFY, thisaf = muadc::currentAF)
...
if (!exists('cont')) {
  cont  <- read.tidy('data/xacontrpt-all-time.csv') %>% 
    mutate(
...
# have to use currentFY -1 so the levels match the training data
classbreaks  <-  c(1945,1955,1965,1985,1995,2010,currentFY-1)
...
h2  <- hallp  %>% 
  ...
  mutate(
    ...
    # , prefemailisMU = grepl('@millikin.edu',prefemail) # low var
    ...
    , isalum          = grepl('(^|~)ALUM|ALMP|ALMM($|~)',dnrc)      # original ALUM only
    # , isalum          = grepl('(^|~)AL($|~)',dnrc)      # original ALUM only, # removed for af19
    # , isalmm          = grepl('(^|~)ALMM($|~)',dnrc)  # combined into AL* due to low variance# removed for af20
    # , isalmp          = grepl('(^|~)ALMP($|~)',dnrc)  ## removed for af20
    , isalmn          = grepl('(^|~)ALMN($|~)',dnrc)
    , isfact          = grepl('(^|~)FAC(T|I|C|P|R|E)($|~)',dnrc) # & !grepl('(^|~)FAC(E|F|R)($|~)',dnrc)
    , isfactformer    = grepl('(^|~)FACF($|~)',dnrc)
    # , isretiree       = grepl('(^|~)FAC(E|R)($|~)',dnrc) # removed for af20
    , isparent        = grepl('(^|~)PRNT($|~)',dnrc)
    , isparentformer  = grepl('(^|~)PRNF($|~)',dnrc)
    , istrustee       = grepl('(^|~)TRU.($|~)',dnrc)# removed for af20
    , isotherdnrc     = grepl('OTHR|OTHM|OTHA|ALMS|ADMH|ALMH|ALMC',dnrc)
    
    , nocontact = grepl('NOC',excl)
    , noother   = grepl('NMC|NMS|NME|NES|NPS|NPC|NFC|NSC', excl)
    , nomail    = grepl('NMC|NMS',excl)
    , noofexcl  = factor(case_when(
    ...
    , numactivities = factor(case_when(
